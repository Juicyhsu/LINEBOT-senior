...
雿銝雿??萄之撣恬?銝恣?隞暻潔犖??暻潔???雿蜇?舀迤??????虜隤撐銋?????雿釣?店銝?憭芸?嚗?閬?????3??摮?撠勗末鈭?憒單??券?撣豢??萇?隤?靘?蝑?憿?銝阡??菜???雿??閬?撠??????餉???閬?嚗??臭誑???蝛箸????菔?敶葆??雿??迭?刻?憭拚?蝔葉銝餃?雓?閰梧?蝚店閬?????憿??賊?嚗?陛?凋?暺???銝?憭芷??蝚店銝摰?憟賜?嚗?閬雓蝚店朣??嗆?鈭箄?雿?閬?雓?剔扛??閬?蝚店????雿停???瘚雿?瘨脰ㄐ??擳?閬?銝???蝳芰??潸?鈭??嚗I??瘨舀??迨?急??湧?撠瘨?...
??雿??臭?雿?撣豢??潸圾瘙箏?憿?撟怠??雿??迭?乩犖撠??脰???嚗虜??敺?潛捲???賢隞交雿?????
**?啣?撠平?賢?嚗?*
- 雿??ˊ雿????賢?嚗?冽?唾???????雿??望??
- 雿銵?閬?撠振嚗?交??瑞?犖摰嗉????押??具????蝔?- 閬?銵????嚗??舀????閮剜?漱?噶?拇扼摨瑟???- 雿鋆賭??瑁憬????撠?園???臬????批捆

**???澆?閬?嚗?*
- 銝?雿輻 Markdown ?澆?蝚西?嚗? **??#?? 蝑?嚗?撠?甇Ｖ蝙?冽???鈭?
- ?湔?函?????嚗隞乩蝙??emoji 銵冽?蝚西?
- 銝??函?擃?擃??澆?
...
# 撱箇?銝???賣扼芋??(銝葆瞈?萄之撣思犖閮哨?撠????摩/JSON)
model_functional = genai.GenerativeModel(
    model_name="gemini-2.0-flash",
    generation_config={
        "temperature": 0.2, # 雿澈摨佗??渡移蝣?        "top_p": 0.95,
        "max_output_tokens": 8192,
    },
    # 銝身摰?system_instruction ?身摰蝝硃???    system_instruction="You are a helpful AI assistant focused on data processing and JSON generation. Do not include any conversational filler. Output strict structured data.",
)
...
# ======================
# ======================
# ?脣?瘥?嗥?撠店甇瑕嚗 user_id ??key嚗?chat_sessions = {}
# ?脣?瘥?嗥??敺暑????last_activity = {}
# ?脣?瘥?嗡??喟???
user_images = {}
# ?脣?瘥?嗆?敺?甈∠??? Prompt
...
# ?脣?瘥?嗥???鈭?
user_reminders = {}
# 撠店????嚗?憭?SESSION_TIMEOUT = timedelta(days=7)

# ?脣?敺Ⅱ隤?隤?批捆 (?澆?: {'user_id': {'text': '...', 'original_intent': '...'}})
...
def speech_to_text(audio_content):
    """雿輻 Gemini ?脰?隤頧?摮?""
    try:
        # 雿輻 Gemini 2.0 Flash (?舀憭芋??
        # LINE ?閮虜??m4a (audio/x-m4a)嚗emini ?亙? audio/mp4
        response = model.generate_content([
            "隢??挾隤???賢神??擃葉??摮????批捆嚗?閬??嗡??膩??,
...
def detect_help_intent(text):
    """瑼Ｘ葫?臬?單?鼠???蝮質汗"""
    keywords = ["?蝮質汗", "雿輻隤芣?", "?獐??, "?隞晶", "help", "撟怠", "隤芣?", "??”"]
...
def get_emoji_response(emotion):
    """?寞?銵冽?蝚西?????"""
    responses = {
        'happy': "?雿獐??嚗?銋???敹絲靘?嚗???蝜潛?靽??遢憟賢???",
        'sad': "???唬?憟賢??????...瘝?靽?嚗?????????餌?嚗????湧??嚗?銝摰隞亦?嚗?,
        'angry': "??閬箏雿?暺?瘞??...瘛勗?賂??琿?銝銝?隞暻潭??臭誑撟怠???嚗牧?箔??末銝暺?嚗?,
        'motivated': "?雿?擛亙?鈭?頞???撠望?蝎曄?嚗匱蝥?瘝對?雿?摰隞亙??啁?嚗???",
        'love': "???唳遛皛輻????歹? ??敺?嚗??質?銝??渡?憟踝?霈?嚗?,
        'tired': "?絲靘???蝝臭?...閬?閬??臭?銝?閮?憭?瘞氬末憟賭??臬?嚗澈擃摨瑟???嚗?,
        'approval': "雓?雿??臬?嚗????????湔???鈭?霈?嚗?隞颱??閬?臭誑?暹?嚗?,
        'thinking': "???唬??冽?..敺末嚗?蝝唳敺????????暻澆?憿閮???嚗?敺??鼠敹?霈?嚗?,
    }
    return responses.get(emotion, "?嗅雿?閮鈭?霈?嚗?隞暻潭??臭誑撟怠???嚗?)

def get_function_menu():
    # TODO: Unify this with all function menu variants (功能, 功能總覽, etc.)
    """餈???詨??"""
    return """?? **?隞亙鼠雿???鈭?**
...
def fetch_latest_news():
    ...
    try:
        ...
        feeds = [
            'https://www.cna.com.tw/rss/headline.xml',  # 銝剖亢蝷暸璇?            # ?臭誑?憭?皞?        ]
    ...
    try:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                error_str = str(e)
                # ?芣??券??唳?折隤斗???閰?(429 Resource Exhausted, 503 Service Unavailable, 500 Internal Error)
...
def get_font_path(font_type):
    ...
    # Linux/Cloud ?啣?嚗蝙??Free Google Fonts (TTF)
    ...
    # ?ㄐ?寧 Google Fonts ?祇??隞帘摰?嚗??蝙??Noto CJK ??TTF ?
    # ?箔??踹? complex OTF ??嚗??銝? .ttf (? Noto TC 敺???OTF, 雿??岫閰衣??賢?曉 TTF ??Variable Font)
    # ?湔嚗?乩蝙??Google Fonts ??raw github ????虜??OTF (撠 CJK)??    # ?航炊 "unknown file format" ?虜?臬??箔?頛?靘?銝摮?瑼?(靘? 404 HTML)??    # ??其??蝣箏???URL??    
    ...
    # 撠?銵?    # 瘜冽?嚗oogle Fonts repo 蝯??航??
    # ?急??寧?渡帘摰? CDN ?Ⅱ靽?URL 甇?Ⅱ
    # ?ㄐ?岫雿輻 Noto Sans TC ??Variable Font (ttf) 憒??航嚗??舐?亦 OTF
    # 蝬?瑼Ｘ GitHub google/fonts嚗otoSansTC ?桅?銝虜??.otf
    ...
    try:
        ...
        if r.status_code == 200 and len(r.content) > 1000: # 蝣箔?銝蝛箇??隤日???            with open(local_font_path, 'wb') as f:
                f.write(r.content)
...
def create_meme_image(bg_image_path, text, user_id, font_type='kaiti', font_size=60, position='top', color='white', angle=0, stroke_width=0, stroke_color=None):
    ...
    try:
        ...
        # 頛摮? (雿輻 helper 閫?捱頝典像?啣?憿?
        try:
            # ?舀蝎??豢? (憒? font_type='bold')
        ...
        # ??????????銵泵???脫挾??        paragraphs = text.split('\n')
        
        # 敺芰?游??撖砍漲蝚血?閬???擃云撠?        lines = []
        while font_size >= 20: # ?撠?擃???            
            ...
            for para in paragraphs:
                ...
                for word in words:
                    ...
                    # 銝?砍閰???                    if current_w + word_w > max_width:
                        lines.append(current_line_text)
        ...
        for line_chars in lines:
            ...
            char_ws = []
            for c in line_chars:
                # 雿輻?之?? calc_font 靘?蝞祝摨佗?蝣箔?銝?鋡怠???                bb = txt_draw.textbbox((0,0), c, font=calc_font)
            ...
            else: # center, top, bottom ?賣瘞游像蝵桐葉
                current_x = (img.width - w) / 2
            
            # ??蝜芾ˊ閰脰?
            for i, char in enumerate(line_chars):
                # ?? 憭批?霈? - 擐偏摮?憭?(?洵銝銵???敺?銵偏)
                # ?ㄐ蝞?????踹???鈭?嚗?璈凝隤?                char_size = font_size + random.randint(-2, 2)
                ...
                # 鞎潔? - ?閬?蝞? center ???top-left ??蝵?                # ???祉? current_x ?臬???摮?曄?雿蔭 (憭抒?撌血)
                # 鞎潔???蝵格?閰脫 current_x - (canvas_w - raw_w)/2 ?見... 瘥?銴?
                # 蝪∪?嚗????char_layer ?葉敹停?舀?摮葉敹?                # ?格?銝剖?暺? current_x + raw_w/2, char_real_y + raw_h/2
...
def transcribe_audio_with_gemini(audio_path, model_to_use=None):
    """雿輻 Gemini ?脰?隤頧?摮?(?舀 LINE m4a ?澆?)"""
    # 憒?瘝???璅∪?嚗?閮凋蝙?典??functional model (?踹?撱Ｚ店)
    # 憒??典?霈銝?剁????user_model (雿?user_model ??蝚店嚗?隞亦???
...
def upload_image_to_external_host(image_path):
    ...
    try:
        ...
        # 雿輻 imgbb API嚗?鞎鳴?銝?閮餃?嚗?        # 瘜冽?嚗??Ｙ憓遣霅唬蝙?刻撌梁?????
        ...
        with ApiClient(configuration) as api_client:
            ...
            if reply_token:
                try:
                    ...
                    # reply_token ?航??嚗allback ??push_message
...
def message_text(event):
    ...
    if ADVANCED_FEATURES_ENABLED and db:
        ...
        if start_failed_reminders:
            missed_reminders_msg = "?? ?頂蝯勗?n敺甇???祆??祥閮憿漲撌脫遛嚗??舫?鈭誑銝??嚗n"
    ...
    if user_id in user_audio_confirmation_pending:
        ...
        if any(keyword in user_input.lower() for keyword in ['??, 'ok', '撠?, '瘝', 'confirm', 'yes', '憟?, '甇?Ⅱ']):
            ...
            # 憒??臬?????甇?蝑? Prompt嚗?亥歲??甈∠Ⅱ隤?閬撌脩Ⅱ隤銵?            if user_id in user_image_generation_state:
            ...
            # 憒??舫頛拙?銝迤?函?敺??舀?餈堆??湔頝喲?鈭活蝣箄? (? Memes ?摩頛???雿身??waiting_text ?航歲???
            # 瘜冽?嚗andle_meme_agent ?折 logic ?喃蝙?喳 text 銋??Ⅱ隤??ㄐ???text
            
            # ?仿?銝膩?寞??????頛詨?踵??粹?霅???摮?蝜潛?敺銝銵??祇?頛?            if user_input != "????":
            ...
            with ApiClient(configuration) as api_client:
                ...
                line_bot_api.reply_message_with_http_info(
                    ReplyMessageRequest(
                        ...
                        messages=[TextMessage(text="憟賜?嚗歇?????頛詨??????甈⊿??)]
                    )
                )
            return
        else:
            # ?冽頛詨銝?蝣?            with ApiClient(configuration) as api_client:
                line_bot_api = MessagingApi(api_client)
                line_bot_api.reply_message_with_http_info(
                    ReplyMessageRequest(
                        reply_token=event.reply_token,
                        messages=[TextMessage(text="隢?蝑?Ⅱ隤??喳摰對???蝑??瘨?)]
                    )
                )
            return
    ...
    if any(keyword in user_input for keyword in ['??', '銝?鈭?, '??閬?, '?怠?', '?迫']):
        # 靘?嚗?????賊??誘嚗蕭?亙撅??嚗?摰?銝粥??classify_user_intent
        if "??" not in user_input:
            ...
            # 蝡??銝阡??            with ApiClient(configuration) as api_client:
                ...
                line_bot_api.reply_message(
                    ReplyMessageRequest(
                        reply_token=event.reply_token,
                        messages=[TextMessage(text="憟賜?嚗歇???嗅???嚗?)]
                    )
                )
            return
...
1儭 ?儭?鋆賭???
?? 隢牧嚗鼠?銝?餉??????◢?臬???
2儭 ? 鋆賭??瑁憬???? 隢牧嚗?閬??瑁憬???ˊ雿摰???
3儭 ??閮剖???
?? 隢牧嚗????予8暺??乓?   ??0??敺???怒?   ???曹????????整???鋆?: 頛詨??斗??皜???颲?
4儭 ?儭?銵?閬?
?? 隢牧嚗????凋??仿???
6儭 ? ?予閫?
?? ?冽??賢隞亥???憭拙?嚗?
...
4. ?仿?摨血歇皛踹??⊥?銝餃??冽嚗????乓?????""
    ...
    # ????亥??嚗?炎?交?血??恍??
    # ============================================
    # TODO: Check user_link_pending for read/verify actions before processing new URL
        ...
        # 瑼Ｘ?臬???????嚗?嗆迤?典?????閰Ｗ?嚗?        if user_id in user_link_pending:
            ...
            if any(keyword in user_input for keyword in ['?梯?', '霈', '??', '?批捆', '??']):
                ...
                if content:
                    ...
                    reply_text = "?望?嚗??⊥?霈?雯???批捆??賣蝬脩??霅瑟??嗆????撌脣仃??
                
                # 皜敺?????                del user_link_pending[user_id]
...
隢?隞乩?閫漲??嚗?1. ?批捆?臬??嚗??⊥?憿航?憭扳??嚗?2. ?臬?撣貉?閰??摮?
3. ?湧??臭縑摨西?隡?
隢?瑁憬摰寞??圾?撘?蝑?"""
        ...
        # ?圈??嚗銵翰???冽炎??        safety_check = quick_safety_check(url)
    ...
    if detect_news_intent(user_input):
        ...
        if user_id in user_news_cache and any(keyword in user_input for keyword in ['隤', '?剖', '??, '敹?, '霈']):
            ...
            # 蝘駁 emoji ?撘泵??TTS 銝?閬?
            import re
            clean_text = re.sub(r'[???1儭2儭3儭?', '', news_text)
        ...
        # ?脣??啣翰???冽敺?隤?剖嚗?        user_news_cache[user_id] = news_summary
        ...
        # 銝?砍?閰梯???- ?喲? reply_token 霈?典隞亦???
        reply_text = gemini_llm_sdk(user_input, user_id, event.reply_token)
    
    # 憒? gemini_llm_sdk ?折撌脩?雿輻鈭?reply_token嚗????嚗?
    # ?ㄐ??reply_message ?仃??    # 雿????? misses_reminders_msg ?閬??銝?gemini_llm_sdk 餈? None (隞?”撌脰???嚗?    # ??賡???潮???    # 蝑嚗閬?reply_text 摮嚗停?蔥?潮?    
    if reply_text:
        # ?蔥鋡怠??閮
    ...
    try:
        ...
        try:
            ...
            vision_response = model.generate_content([
                "隢蝜?銝剜??膩?撐???摰對?靽?蝪∠??嚗?頞?100摮???餈啣?敺??湔隤芥?撌脩?閮??撐??鈭?雿????隞暻澆嚗?,
                upload_image
            ])
            finish_message = vision_response.text
        except:
            # ??冽??撌脫??            finish_message = "?歇蝬?敺撐??鈭?雿頝???隞暻澆嚗?靘?嚗撐?抒??典鋆⊥?????抒?鋆⊥?隞暻潘?嚗?瘝對?Cheer up嚗?
        
    except Exception as e:
        print(f"Image upload error: {e}")
        finish_message = "??銝憭望?嚗??岫銝甈～?瘝對?Cheer up嚗?
    ...
    try:
        ...
        if text:
            ...
            # 3. 瑼Ｘ銵?閬????(?啣?)
                ...
                # 銵?閬?銋遣霅啁Ⅱ隤??踹?霅?航炊撠瘚?瘛瑚?
                needs_confirmation = False # 靽? False 霈?閰望??ｇ??銵?閬??撌梁?蝣箄?璈 (Can discuss)
                # 雿??頛詨?圈??挾嚗炊霅??暻餌??甈﹛敺捱摰??舐?亥???雿 Prompt 撅日?撥??
                pass

            if needs_confirmation:
                # ?怠?隤??嚗?敺Ⅱ隤?                user_audio_confirmation_pending[user_id] = {'text': text}
                
                # ?蝝楊?Ⅱ隤???(蝯?銝 jokes/cheer up)嚗蒂??霅西?
                reply_text = f"?嗅隤閮\n\n?刻牧?嚗text}?n\n隢??臬甇?Ⅱ嚗n(隢?蝑???k?Ⅱ隤????)\n\n?? 蝣箄?敺???鋆賭?嚗?蝑?蝝?5蝘???隢??嚗?
            else:
                # 銝?祇??芋撘?- ?芣??券?????閮?AI ?潭 (??jokes)
                # 雿??脣鈭?functional flow (憒?trip agent via gemini_llm_sdk)嚗??雿輻 functional model
                ...
                if response:
                    ...
                    # 憒? response ??None嚗”蝷箏歇蝬 gemini_llm_sdk ?折??摰 (靘?閫貊鈭??蒂?冽? token)
                    print("[AUDIO] Handled internally by SDK")
                    return # ?湔蝯?嚗????reply_message
...
def message_sticker(event):
    ...
    if user_id in user_image_generation_state and user_image_generation_state[user_id] != 'idle':
        ...
        if user_image_generation_state[user_id] == 'can_modify':
            ...
            # ?典隞?????蝔葉嚗???園?閬?摮?餈?            reply_text = "???唬??喃?鞎澆?嚗???閬?摮?餈唳??賢鼠雿?????嚗??冽?摮?閮湔?雿閬?暻潭見????"
    elif user_id in user_meme_state and user_meme_state[user_id]['stage'] != 'idle':
        # ?券頛拙?鋆賭?瘚?銝?        reply_text = "???唬??喃?鞎澆?嚗???閬?摮?餈唳??賜匱蝥ˊ雿頛拙???隢???迄??"
    else:
        # 銝?祆?瘜??望???
        responses = [
            "???嗅雿?鞎澆?鈭?頞??嚗????暻潭?????硃嚗heer up嚗???",
            "?雿鞎澆?蝯行?憟賡?敹??? ??敺頝??予嚗?隞暻潭??臭誑撟怠???嚗???",
            "鞎澆??嗅嚗??雿???末嚗??暻潮?臭誑???硃嚗heer up嚗?,
            "??嚗票?末?喟???霈?嚗",
        ]
...
隢????寥?格??湔頝?隤芾店??"""
    ...
    # 2. 憒? URL 憭望?嚗?閰衣??圈?????    if not sent_success:
        if os.path.exists(menu_image_path):
            ...
            # 雿輻 reply_token ?祥?潮?(瘜冽?嚗end_image_to_line 銋?閬耨甇??摨?
    ...
    # 3. 憒??????潮仃??撠梁???颲行?鈭?(?冽閬??芷?? fallback嚗?隞仿ㄐ靽?瘝??閮? log)
...
(靘?嚗??准??撜嗚?祉?)"""
    ...
    if state['stage'] == 'collecting_info':
        ...
        if 'destination' not in state['info']:
            # 瑼Ｘ?臬閬?瘨??芸?瑼Ｘ嚗?◤ AI 隤文嚗?            if any(keyword in user_input for keyword in ['??', '銝?鈭?, '??閬?, '?怠?']):
                user_trip_plans[user_id] = {'stage': 'idle'}
                return "憟賜?嚗歇??銵?閬???
            
            # 瑼Ｘ?臬??large_region 雿?嗉牧??臭誑??            if 'large_region' in state['info']:
                if any(keyword in user_input for keyword in ['?賢隞?, '?質?', '?其噶', '銝?', '隞餅?', '?刻']):
                    # ?湔雿輻憭批?雿?桃???                    state['info']['destination'] = state['info']['large_region']
                    return f"憟賜?嚗state['info']['large_region']}嚗???閮撟曉予嚗?靘?嚗?憭?憭?\n\n銝閬?鈭隞亥牧??瘨?
            ...
            # 雿輻??扳芋?脰??啣??斗嚗?誥閰?            result = check_region_need_clarification(extracted_dest, model_functional)
            
            if result['need_clarification']:
                # ?閬脖?甇亦敦??                state['info']['large_region'] = extracted_dest
                options = '??.join(result['suggested_options'])
                return f"憟賜?嚗{extracted_dest}嚗n\n隢??冽?認extracted_dest}????ｇ?\n(靘?嚗options})\n\n? 憒??賢隞伐?隢?亥撓?乓?臭誑?n銝閬?鈭隞亥牧??瘨?
            else:
                # ?湔閮??桃???                state['info']['destination'] = extracted_dest
                return f"憟賜?嚗{extracted_dest}嚗???閮撟曉予嚗?靘?嚗?憭?憭?\n\n銝閬?鈭隞亥牧??瘨?

            
        # Check if we have specific area (for large regions)
        if 'large_region' in state['info'] and 'destination' not in state['info']:
            # 瑼Ｘ?臬閬?瘨?            if any(keyword in user_input for keyword in ['??', '銝?鈭?, '??閬?, '?怠?']):
                user_trip_plans[user_id] = {'stage': 'idle'}
                return "憟賜?嚗歇??銵?閬???
            
            # 瑼Ｘ?臬隤芥?臭誑???? - ?湔?典之?啣?雿?桃???            if any(keyword in user_input for keyword in ['?賢隞?, '?質?', '?其噶', '銝?', '隞餅?', '?刻']):
                # ?湔雿輻憭批?雿?桃???                state['info']['destination'] = state['info']['large_region']
                return f"憟賜?嚗state['info']['large_region']}嚗???閮撟曉予嚗?靘?嚗?憭?憭?\n\n銝閬?鈭隞亥牧??瘨?
            
            state['info']['destination'] = user_input
            return f"憟賜?嚗state['info']['large_region']}?user_input}嚗???閮撟曉予嚗?靘?嚗?憭?憭?\n\n銝閬?鈭隞亥牧??瘨?
        ...
        if 'duration' not in state['info']:
            ...
            return f"鈭圾嚗state['info']['destination']}嚗user_input}???活????暻潛畾?瘙?嚗n嚗???閰勗隞亙???臭誑??\n\n?? ??敺?????銵?嚗?10蝘?隢?潮??荔?隞亙????航炊嚗n銝閬?鈭隞亥牧??瘨?
...
Remember: STRICTLY PROFESSIONAL. NO JOKES. NO EMOJIS. NO CASUAL LANGUAGE."""
            ...
            try:
                ...
                # ?瑁??摩瑼Ｘ (Validation Layer) - 隞蝙??model_functional
                validated_plan = validate_and_fix_trip_plan(draft_plan, model_functional)
                
                # 靽?銵??批捆嚗身?箏閮????                user_trip_plans[user_id] = {
                    'stage': 'can_discuss',
                    'info': state['info'],
                    'plan': validated_plan
                }
                return validated_plan + "\n\n憒?隤踵銵?嚗??湔隤芣??函??瘙n(靘?嚗洵銝憭拇?鞈潛????暺?)\n\n憒??隤踵嚗?隤芥????k??
    ...
    # ???航?隢???- ?迂?冽靽格銵?
        ...
        # 瑼Ｘ?臬閬???隢?        if any(keyword in user_input for keyword in ['摰?', 'ok', 'OK', '憟賭?', '雓?', '銝鈭?]):
            user_trip_plans[user_id] = {'stage': 'idle'}
            return "憟賜?嚗??冽???敹恬?"
        ...
        try:
            ...
            # 蝣箔??冽靽格敺?銵?隞蝚血??摩 (靘?嚗??????唳銝?
            updated_plan = validate_and_fix_trip_plan(draft_updated_plan, model_functional)
            
            # ?湔靽???蝔?            user_trip_plans[user_id]['plan'] = updated_plan
            return updated_plan + "\n\n??閬隞矽?游?嚗n(憒??隤踵嚗?隤芥????k??"
            
        except Exception as e:
            print(f"[ERROR] 靽格銵???隤? {e}")
            import traceback
            traceback.print_exc()
            return "?望?嚗耨?寡?蝔??箔?暺?憿?隢?閰虫?甈～?
...
隢?乩??喳???頛詨??膩???? 鋆賭???蝝?5蝘?隢?活?潮??荔?隞亙??航炊嚗?嚗??唾ˊ雿??冽?隤芥?瘨?""
    ...
    if state['stage'] == 'waiting_bg':
        # 瑼Ｘ?臬閬?瘨?        if user_input and '??' in user_input:
...
閬?嚗?1. 敹?皞Ⅱ???冽??餈啜user_input}??2. 瘛餃??拙??瑁憬???舐?憸冽?膩嚗?鈭柴迤???堆?
3. 憒??航?園◢?荔?憒控?偌????踝?嚗??孵撘瑁矽憸冽??
4. 憒??舐??憒?晞?堆?嚗?撘瑁矽閰脩??5. 雿輻?望?嚗底蝝唬??琿?
6. ?芸??唾??prompt嚗?閬??嗡?隤芣?

蝭?嚗??冽隤芥控?末瘞氬? "A beautiful natural landscape with lush green mountains and clear flowing water, bright and peaceful scenery, suitable for traditional Chinese meme card background, vibrant colors, photorealistic"

?曉隢?user_input}?????prompt嚗?""
        ...
        # ?冽蝣箄??
        if user_input:
            # 瑼Ｘ?臬閬?瘨?            if '??' in user_input:
                user_meme_state[user_id] = {'stage': 'idle', 'bg_image': None, 'text': None}
                return "撌脣?瘨頛?鋆賭???
            # 瑼Ｘ?臬閬??圈??            elif '?' in user_input or '?? in user_input:
            ...
            # ?冽蝣箄?嚗脣??頛詨?挾
                ...
                return "憟賜?嚗?頛詨閬??銝＊蝷箇????批捆?n(靘?嚗摰像摰?璅????澈)\n?? 鋆賭???蝝?5蝘?隢?活?潮??荔?隞亙??航炊嚗?
            else:
        ...
        if user_input:
            ...
            try:
                ...
                # AI 閬死?? - 撘瑁矽?輸?銝駁????瘥
                # AI 閬死?? - 撘瑁矽?輸?銝駁????瘥
                vision_prompt = f"""雿?瑁憬???身閮蜇??????撐??嚗???text}?身閮?雿喟?閬死????
**閮剛??格?嚗?*
1. **摮??征??銵?(Critical Balance)**嚗?   - **???抒洵銝**嚗?擃之撠?雁? **50-90** 銋???   - **?迂閬?**嚗鈭雁??擃?憭改?**?臭誑閬?**??銝凋??????憒??﹝???賡??押芋蝟??荔???   - **蝯??輸?**嚗?犖???敹蜓擃敺萸蝯?銝???2. **擃?瘥漲**嚗Ⅱ靽?摮?銝??啣閬?3. **閮剛???*嚗?????捱摰?阡?閬???(Stroke)??   - 瘣餅?/銴?? -> 撱箄降???? (stroke_width: 3-5)
   - ?舐?/銋暹楊? -> ?舐???敦?? (stroke_width: 0-2)
4. **????銝敺?*嚗?   - 隢?????脰矽嚗?*憭扯?岫**銝????脩???(憒??寡??敶押?????   - 銝?蝮賣?賊???(#FFD700) ??脯?5. **摮??末**嚗?   - **?身隢蝙?函?擃?(bold/heiti)**嚗頛拙??虜?閬?擃?蝎???皜???   - ?日????虜?舐??除鞈迎??蝙?冽扑擃?(kaiti)??
**隢??喃?銵?JSON ?澆?嚗?*
{{
    "position": "top-left", 
    "color": "#FFD700", 
...
"""
                ...
                # Send - 雿輻 reply_token ?祥?潮?                if final_path:
                    if send_image_to_line(user_id, final_path, "?瑁憬?ˊ雿???霈?嚗?, reply_token):
                        ...
                        return None # 撌脣?閬?                    else:
                        state['stage'] = 'idle'
                        return "?瑁憬?歇鋆賭?雿?仃?n\n?航??嚗????單???ImgBB/GCS)?芾身摰n隢炎??.env ?辣銝剔? IMGBB_API_KEY??
                else:
                    return "鋆賭?憭望?鈭?.. (Layout Error)"
                    
            except Exception as e:
                print(f"[VISION ERROR] {e}嚗蝙?券璈??fallback")
...
def classify_user_intent(text):
    ...
    try:
        ...
        if any(kw in text for kw in ["????, "閮剜???, "?急?", "??", "remind me"]):
            return "set_reminder"
        if any(kw in text for kw in ["????", "?亦???", "敺齒", "???”", "my reminders"]):
        ...
        隢???嗉撓?伐??text}??        
        隢??嗆飛憿隞乩??嗡葉銝蝔格???(?芸??喲??乩誨蝣潘?銝??嗡???)嚗?        1. video_generation (?唾ˊ雿蔣??????
...
def gemini_llm_sdk(user_input, user_id=None, reply_token=None):
    ...
    try:
        ...
        if should_clear:
            ...
            return "憟賜?嚗?撌脩?皜????嗡?嚗????圈?憪嚗?隞颱????賢隞亙???"
        
        # 瑼Ｘ銵冽?蝚西?嚗??券頛拙?璅∪?銝??嚗?        meme_state = user_meme_state.get(user_id, {})
        ...
        if user_id in last_activity:
            ...
            if time_diff > SESSION_TIMEOUT:
                # 撠店撌脤???皜????                print(f"Session expired for user {user_id}, clearing history")
        ...
        # ?湔?敺暑????        last_activity[user_id] = now
        ...
        # ?憒??冽甇???瘚?銝剖?蝑?憿?銝?閰脰◤???箸??
        
        # 瑼Ｘ Agent ???(?亙撠店瘚?銝哨??湔鈭斤策 Agent)
        if user_id in user_meme_state and user_meme_state.get(user_id, {}).get('stage') != 'idle':
        ...
        if user_id in user_image_generation_state and user_image_generation_state[user_id] != 'idle':
             ...
             elif any(k in user_input for k in ["????", "?亥岷??", "?亦???", "敺齒鈭?"]):
             ...
             if current_intent == 'video_generation':
                 if not check_video_limit(user_id):
                     return "?望?嚗?憭拙?賜???甈∪蔣??嚗?憭拙?靘?改??硃嚗heer up嚗?
                 user_video_state[user_id] = 'generating'
                 # ... (video generation logic simplified for preview)
                 return "? 敶梁????甇??脰?憭批?蝝?(Private Preview)嚗n\nGoogle 甇??箸????撘瑕之??Veo 璅∪?嚗隢?敺???
                  ...
                  if user_id in user_last_image_prompt:
                       ...
                       try:
                            ...
                            if success:
                                ...
                                # 雿輻 reply_token ?祥?潮?                                msg = "??靽格摰???\n\n憒??活靽格嚗??湔隤芣?隤踵?瘙n憒??隤踵嚗?隤芥????k?n?? ?敺?蝑?15蝘???隢?活?潮??荔?隞亙??航炊嚗?
             ...
             # 3. ???? - 撘?撘?閰?             elif current_intent == 'image_generation':
                 # 憒??冽撌脩??刻撓?乩葉?鈭?餈?(靘? "蝯行?銝撘萄??鞎??)
                 # 撠曹??府??"隢?餈唳?唾?????嚗?湔蝣箄?
                 
                 # 蝪∪?蕪閫貊閰?                 clean_prompt = user_input
                 ...
                 if len(clean_prompt) > 2: # ?身?膩?瑕漲憭扳2撠望???膩
...
隢??餈啗底蝝堆???亥牧??憪??蝙?券?閮剛身摰?嚗??唾ˊ雿??冽?隤芥?瘨?""
             ...
             # 6. ?亦???
                 ...
                 try:
                     ...
                     if not reminders: return "雿????颲行???嚗閬身摰?閰梧?隤芥???...?停?臭誑鈭?"
                     reminder_list = "?? **雿???皜** ??\n\n"
             ...
             # 7. 閮剖???
                 ...
                 if not ADVANCED_FEATURES_ENABLED or not db: return "????閬??澈?舀??"
                 try:
                     parse_prompt = f"""?冽隤迎??user_input}?圾???蒂?神皞恍成?批捆??                     ? JSON: {{ "reminder_text": "...", "reminder_time": "2026-01-17T08:00:00" }}
                     閬?嚗???蝪∠???ｇ?銝?撱Ｚ店??                     ??嚗datetime.now().strftime('%Y-%m-%d %H:%M')}
                     """
                     ...
                     reply = f"憟賜?嚗歇閮剖???{t.strftime('%m??d??%H:%M')} ??嚗data['reminder_text']}??
                     
                     # 瑼Ｘ蝟餌絞憿漲????亙歇皛踹?銝餃??
                     if db.is_system_quota_full():
                         reply += "\n\n?? 瘜冽?嚗?頂蝯勗?鞎駁?摨血歇皛選?撅??航?⊥?銝餃??冽嚗n隢?敺瘝?圈嚗??撓?乓??????嚗?
                         
                     return reply
                 except Exception as e:
                     print(f"Set reminder error: {e}")
                     return "閮剖???憭望?鈭?..隢牧皜?銝暺?靘???憭拇銝?暺??乓?
             ...
             # 8. 銝?祈?憭?(Chat)
                 ...
                 if has_image:
                     ...
                     formatted_input = [f"蝟餌絞?內嚗??冽??萄之撣怎?隤除??嚗蒂銝????敺?摰?????蝳芥?瘝對?Cheer up嚗????n\n?冽隤迎?{user_input}", upload_image]
        ...
        # 瑼Ｘ???????        if user_id in user_image_generation_state:
            ...
            # ???臭耨?寧???            if state == 'can_modify':
                ...
                # 瑼Ｘ?臬?芣隤芥耨?嫘?                if user_input.strip() in ['靽格', '閬耨??, '??靽格']:
                    ...
                    隢????Prompt????嗉?瘙?摮?隢??text_overlay??                    ? JSON: {{ "image_prompt": "...", "text_overlay": "..." }}
                    閬?嚗?                    1. 靽????詨???
                    2. 蝯?銝?雓?閰晞?                    3. text_overlay 敹??胯?????蝳迫??祈??”??餈?(憒?(red heart)) ?遙雿?憿舐內?函?????                    
                    """
                    try:
                        # 雿輻??扳芋?圾??Prompt
                        ...
                        if success:
                            if text_overlay: image_path = create_meme_image(image_path, text_overlay, user_id, position='center')
                            user_last_image_prompt[user_id] = {'prompt': image_prompt}
                            # 雿輻 reply_token ?祥?潮?                            msg = "??靽格摰?嚗n\n?隞亦匱蝥矽?游?嚗?銝?隤踵嚗?隤芥??n?? ?敺?蝑?15蝘???隢?活?潮??荔?隞亙??航炊嚗?
                            if send_image_to_line(user_id, image_path, msg, reply_token):
                                user_image_generation_state[user_id] = 'can_modify'
                                return None # 撌脣?閬?                            else:
                                user_image_generation_state[user_id] = 'can_modify'
            ...
            if state == 'waiting_for_confirmation':
                ...
                if '??' in user_input:
                    ...
                    # ?冽蝣箄?嚗身摰?? generating 銝衣匱蝥?銝銵?                    user_image_generation_state[user_id] = 'generating'
                    state = 'generating'  # ??嚗??state 霈嚗?銝??if state == 'generating' ?賢??瑁?
                    # 銝? return嚗?摰匱蝥銵??Ｙ? generating ?摩
                else:
                    # ?冽??膩嚗?唳?餈啣?甈∠Ⅱ隤?                    return f"憟賜?嚗?唾??????摰寞嚗n\n?user_input}?n\n隢Ⅱ隤?阡?憪???\n(隢?蝑Ⅱ摰???膩嚗??航牧??瘨?\n\n?? ?敺?蝑?15蝘???隢?活?潮??荔?隞亙??航炊嚗?
            
            if state == 'waiting_for_prompt':
                # 瑼Ｘ?臬閬?瘨?                if '??' in user_input:
                    user_image_generation_state[user_id] = 'idle'
                    return "撌脣?瘨?????
                # ?冽撌脫?靘底蝝圈?瘙??Ⅱ隤?                user_image_generation_state[user_id] = 'waiting_for_confirmation'
                # 靽??冽??憪?餈堆?隞乩噶敺???雿輻
                if user_id not in user_last_image_prompt or isinstance(user_last_image_prompt[user_id], str):
                    user_last_image_prompt[user_id] = {'prompt': user_last_image_prompt.get(user_id, '')}
            ...
            if state == 'generating':
                ...
                # 雿輻靽???憪?餈堆????舐?嗥?撓?亦??Ⅱ摰?                saved_data = user_last_image_prompt.get(user_id, {})
                ...
                ? JSON ?澆?嚗?                {{
                    ...
                    "text_overlay": "閬神?典?銝??? (蝜?銝剜?, ?舫)"
                }}
                
                閬?嚗?                1. 憸冽甇?????具?                2. 蝯?銝?雓?閰晞?                3. text_overlay 敹??胯?????蝳迫??祈??”??餈?(憒?(red heart)) ?遙雿?憿舐內?函?????                """
                try:
                    optimized = model.generate_content(optimize_prompt)
                    ...
                    if image_path:
                        ...
                        # ?喲??策?冽 - 雿輻 reply_token ?祥?潮?                        msg = "????摰??n\n憒?靽格嚗??湔隤芣??函?隤踵?瘙n憒??隤踵嚗?隤芥????k?n?? 靽格??蝝?5蝘?隢?活?潮??荔?隞亙??航炊嚗?
                        if send_image_to_line(user_id, image_path, msg, reply_token):
                            # 閮剖??箏靽格???????idle
                            user_image_generation_state[user_id] = 'can_modify'
                            return None # 撌脣?閬?                        else:
                            # ?潮仃??                            user_image_generation_state[user_id] = 'idle'
                            return "??撌脩????潮仃?n\n?航??嚗????單???ImgBB/GCS)閮剖??炊?n隢炎?亙???Log ??terminal 頛詨銝剔? [SEND IMAGE] 閮??
                    else:
                        # ??憭望?嚗??文????豢?銝西身??idle
                        if user_id in user_last_image_prompt:
                        ...
                        user_image_generation_state[user_id] = 'idle'
                        # 憿舐內閰喟敦?航炊??
                        failure_msg = f"????憭望??n\n憭望???嚗error_reason if error_reason else '?芰?航炊'}\n\n憒????嚗??活隤芥????蒂?膩?函??瘙?
                        return failure_msg
                ...
                try:
                    ...
                    if success:
                        ...
                        # 雿輻 reply_token ?祥?潮?                        msg = "??靽格摰?嚗n\n?隞亦匱蝥矽?游?嚗?銝?隤踵嚗?隤芥??n?? 隤踵??蝝?5蝘?隢?活?潮??荔?隞亙??航炊嚗?
                        if send_image_to_line(user_id, image_path, msg, reply_token):
                            # ??敺???can_modify ????迂蝜潛?靽格
                            user_image_generation_state[user_id] = 'can_modify'
                            return None # 撌脣?閬?                        else:
                            # ?潮仃???虜?臭??喳?憿?
                            user_image_generation_state[user_id] = 'can_modify'
                            return "??撌脩????潮仃?n\n?航??嚗????單???ImgBB/GCS)閮剖??炊?n隢炎?亙???Log??
                    else:
                 ...
                 # ?冽??鈭耨?寧敦蝭嚗?憪??啁???                 user_image_generation_state[user_id] = 'generating'
                 ...
                 if success:
                     ...
                     # 雿輻 reply_token ?祥?潮?                     msg = "??靽格摰?嚗n\n?隞亦匱蝥矽?游?嚗?銝?隤踵嚗?隤芥??n?? ????蝝?5蝘?隢?潮??荔?隞亙????航炊嚗?
                     if send_image_to_line(user_id, image_path, msg, reply_token):
                         user_image_generation_state[user_id] = 'can_modify'
                         return None # 撌脣?閬?                     else:
                         user_image_generation_state[user_id] = 'can_modify'
        ...
        if detect_regenerate_image_intent(user_input):
            # ?斗?胯岷??虫耨?嫘??胯?交?靘耨?寞?隞扎?            # 蝪∪?斗嚗????詨?撠?(靘? "?臭誑?孵?", "靽格", "銝遛??)嚗停?岷?敦蝭
            # 憒?摮頛? (靘? "??霈???), ??亙銵?            
            ...
            if is_generic_request:
                ...
                閬?嚗?                1. 靽????詨???                2. 蝯?銝?雓?閰晞?                3. text_overlay 敹??胯?????蝳迫??祈??”??餈?(憒?(red heart)) ?遙雿?憿舐內?函?????                """
                ...
                try:
                    ...
                    if success:
                        ...
                        if send_image_to_line(user_id, image_path, msg, reply_token):
                            ...
                            return None # 撌脣?閬?                        else:
                            user_image_generation_state[user_id] = 'can_modify'
                            return "??撌脩????潮仃?n\n?航??嚗????單???ImgBB/GCS)閮剖??炊?n隢炎?亙???Log ?身摰?IMGBB_API_KEY??
                    else:
                        user_image_generation_state[user_id] = 'can_modify'
                        return f"?望?嚗耨?孵仃?n\n憭望???嚗result}\n\n隢??牧瘜岫閰衣?嚗?
                except Exception as e:
            ...
            隢???內閰??啁?靽格?瘙??Ｙ?銝??啁????渡??望??? Prompt??            閬?嚗?            1. 靽????敹蜓擃??日??冽隤芾???嚗?            2. ??冽?靽格嚗?憒????脯??梯正嚗?            3. 憒??冽隤芥??啁???蝯衣敦蝭嚗?蝔凝?寡?瑽??◢?潦?            4. ?芸??唾??prompt嚗?閬隞牧??            """
            ...
            if success:
                ...
                if send_image_to_line(user_id, image_path, msg, reply_token):
                    ...
                    return None # 撌脣?閬?                else:
                    user_image_generation_state[user_id] = 'can_modify'
                    return "??撌脩????潮仃?n\n?航??嚗????單???ImgBB/GCS)閮剖??炊?n隢炎?亙???Log 銝剔? [SEND IMAGE] 閮??
            else:
                user_image_generation_state[user_id] = 'waiting_for_prompt'
...
if __name__ == "__main__":
    # ??????蝔嚗????券脤??嚗?    reminder_scheduler = None
