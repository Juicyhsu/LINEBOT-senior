...
＊不想製作了隨時說「取消」"""
    ...
    if state['stage'] == 'waiting_bg':
        ...
        if image_content:
            ...
            # 不發送圖片給用戶
            return "已收到背景圖片。\n\n請輸入要在圖片上顯示的文字內容。\n(例如：早安、平安喜樂、認同請分享)\n⚠️ 送出訊息後，製作期間約30秒內，請勿發送訊息，以免造成錯誤！"
...
現在請為「{user_input}」生成英文 prompt："""
             ...
             try:
                 ...
                 if success:
                     ...
                     send_image_to_line(user_id, result, "", reply_token)
...
範例2：top-left,rainbow,kaiti,-8"""
            ...
            if final_path:
                if send_image_to_line(user_id, final_path, "", reply_token):
                    state['stage'] = 'idle'
                  ...
                  if user_id in user_last_image_prompt:
                       ...
                       try:
                            ...
                            if success:
                                ...
                                # 使用 reply_token 免費發送
                                if send_image_to_line(user_id, image_path, "", reply_token):
                                    user_image_generation_state[user_id] = 'can_modify'
                                    return "Cheer up！圖片修改完成🎉\n\n如需再次修改，請直接說明調整需求。\n如不需調整，請說「完成」或「ok」。\n⚠️ 送出訊息後，生成期間約30秒內，請勿發送訊息，以免造成錯誤！"
                                else:
                                    return "圖片生成成功但發送失敗。"
                            else:
                                user_image_generation_state[user_id] = 'can_modify'
