...
def get_emoji_response(emotion):
    """根據表情符號情緒回應"""
    responses = {
        'happy': "看到你這麼開心，我也跟著開心起來了！讚喔！繼續保持這份好心情！加油！Cheer up！",
        'sad': "我看到你好像有點難過...沒關係的，不開心的事情都會過去的！我會一直陪著你！加油！你一定可以的！Cheer up！",
        'angry': "我感覺到你有點生氣了...深呼吸，冷靜一下。有什麼我可以幫忙的嗎？說出來會好一點喔！加油！Cheer up！",
        'motivated': "看到你的鬥志了！超棒的！就是這股精神！繼續加油！你一定可以做到的！讚喔！Cheer up！",
        'love': "感受到滿滿的愛！❤️ 真的很棒！愛能讓世界更美好！讚喔！Cheer up！",
        'tired': "看起來你有點累了...要不要休息一下？記得多喝水、好好休息喔！身體健康最重要！加油！Cheer up！",
        'approval': "謝謝你的肯定！👍 有你的支持我更有動力了！讚喔！有任何需要都可以找我！Cheer up！",
        'thinking': "我看到你在思考...很好！仔細思考是很棒的習慣！有什麼問題想討論的嗎？我很樂意幫忙！讚喔！",
    }
    return responses.get(emotion, "收到你的訊息了！讚喔！有什麼我可以幫忙的嗎？加油！Cheer up！")
...
def transcribe_audio_with_gemini(audio_path):
    ...
    try:
        ...
        # 請 AI 轉錄
        prompt = "請逐字轉錄這段語音的內容，只輸出轉錄後的文字，不要加其他描述或問候語。"
...
👉 隨時都可以跟我聊天喔！"""
    ...
    # 這裡的 reply_message 會失敗。但這是預期行為 - 結果會透過 push_message 發送。
    # 所以我們用 try-except 包起來，失敗時不做任何事。
    try:
        with ApiClient(configuration) as api_client:
            line_bot_api = MessagingApi(api_client)
            line_bot_api.reply_message_with_http_info(
        ...
        # reply_token 可能已被使用，這是正常的（狀態通知已發送）
        print(f"[REPLY] Token possibly already used: {e}")
...
def message_image(event):
    ...
    try:
        ...
        if user_id in user_meme_state and user_meme_state[user_id].get('stage') == 'waiting_bg':
             ...
             # 呼叫 agent 處理
             reply_text = handle_meme_agent(user_id, image_content=image_data)
...
def message_audio(event):
    ...
    try:
        ...
        if text:
            ...
            reply_text = "抱歉，我沒聽清楚你說的話，可以再說一次嗎？加油！Cheer up！"
        
    except Exception as e:
        print(f"Audio processing error: {e}")
        reply_text = "語音處理失敗，請再試一次。加油！Cheer up！"
...
Remember: STRICTLY PROFESSIONAL. NO JOKES. NO EMOJIS. NO CASUAL LANGUAGE."""
            ...
            try:
                # 發送狀態通知（使用 reply_message，免費）
                send_status_notification(reply_token, "正在為您撰寫詳細行程，請稍候...")
...
＊不想製作了隨時說「取消」"""
    ...
    if state['stage'] == 'waiting_bg':
        ...
        # Handle Text Description for Generation
             ...
             # Generate background
             # 發送狀態通知（使用 reply_message，免費）
             send_status_notification(reply_token, "正在生成背景圖片，請稍候...")
...
現在請為「{user_input}」生成英文 prompt："""
             ...
             try:
                 ...
                 if success:
                     ...
                     # 發送背景圖給用戶確認
                     send_image_to_line(user_id, result, "這是生成的背景圖")
        ...
        if user_input:
            ...
            # 發送狀態通知（使用 reply_message，免費）
            send_status_notification(reply_token, "正在製作長輩圖，請稍候...")
...
範例2：top-left,rainbow,kaiti,-8"""
            ...
            # Send
            if final_path:
                if send_image_to_line(user_id, final_path, ""):
                    state['stage'] = 'idle'
                    return "長輩圖製作完成，讚喔！Cheer up！"
                else:
                    state['stage'] = 'idle'
...
def gemini_llm_sdk(user_input, user_id=None, reply_token=None):
    ...
    try:
        ...
        if should_clear:
            ...
            return "好的！我已經清除所有記憶了，讓我們重新開始吧！加油！Cheer up！讚喔！有任何問題都可以問我～"
        ...
        if user_id in user_meme_state and user_meme_state.get(user_id, {}).get('stage') != 'idle':
             return handle_meme_agent(user_id, user_input)
             
        if user_id in user_trip_plans and user_trip_plans.get(user_id, {}).get('stage') != 'idle':
             return handle_trip_agent(user_id, user_input)
        ...
        if user_id in user_image_generation_state and user_image_generation_state[user_id] != 'idle':
             ...
             # 2. 圖片修改 (與 Image Gen 分開處理)
                  ...
                  if user_id in user_last_image_prompt:
                       ...
                       # 獲送狀態通知（使用 reply_message，免費）
                       send_status_notification(reply_token, "收到修改指令！馬上幫你重畫...✨")
                       ...
                       try:
                            ...
                            if success:
                                ...
                                if send_image_to_line(user_id, image_path, "你的圖片已更新！"):
...
請盡量描述詳細，或直接說「開始生成」使用預設設定。\n＊不想製作了隨時說「取消」。"""
             ...
             # 4. 長輩圖製作
                 ...
                 return handle_meme_agent(user_id, user_input, is_new_session=True)

             # 5. 行程規劃
             elif current_intent == 'trip_planning':
                 return handle_trip_agent(user_id, user_input, is_new_session=True)
             ...
             # 7. 設定提醒
                 ...
                 try:
                     ...
                     return f"好的！我會在 {t.strftime('%Y年%m月%d日 %H:%M')} 提醒你「{data['reminder_text']}」！\n一定會準時通知！"
                 except Exception as e:
        ...
        if user_id in user_image_generation_state:
            ...
            if state == 'can_modify':
                ...
                if user_input.strip() in ['修改', '要修改', '我要修改']:
                    ...
                    user_image_generation_state[user_id] = 'generating'
                    # 發送狀態通知（使用 reply_message，免費）
                    send_status_notification(reply_token, "正在修改圖片，請稍候...✨")
                    ...
                    try:
                        ...
                        if success:
                            if text_overlay: image_path = create_meme_image(image_path, text_overlay, user_id, position='center')
                            user_last_image_prompt[user_id] = {'prompt': image_prompt}
                            if send_image_to_line(user_id, image_path, "修改後的圖片"):
                                user_image_generation_state[user_id] = 'can_modify'
                                return "Cheer up！圖片修改完成🎉\n\n如需再次修改，請直接說明調整需求。\n如不需調整，請說「完成」或「ok」。"
                            else:
                                user_image_generation_state[user_id] = 'can_modify'
            ...
            if state == 'generating':
                # 用戶已確認，開始生成
                # 發送狀態通知（使用 reply_message，免費）
                send_status_notification(reply_token, "正在生成圖片，請稍候 30 秒到 1 分鐘...")
                ...
                try:
                    ...
                    if image_path:
                        ...
                        # 傳送圖片給用戶
                        if send_image_to_line(user_id, image_path, "你的圖片完成了！"):
                            # 設定為可修改狀態，而不是 idle
                            user_image_generation_state[user_id] = 'can_modify'
                            return "圖片生成完成。\n\n如需修改，請直接說明您的調整需求。\n如不需調整，請說「完成」或「ok」。"
                        else:
                ...
                # 發送狀態通知（使用 reply_message，免費）
                send_status_notification(reply_token, "收到修改指令！馬上幫你調整...✨")
                ...
                try:
                 ...
                 # 發送狀態通知（使用 reply_message，免費）
                 send_status_notification(reply_token, "收到修改細節！這就幫你調整...✨")
                 ...
                 if success:
                     ...
                     if send_image_to_line(user_id, image_path, "你的圖片已更新！"):
                         user_image_generation_state[user_id] = 'can_modify'
                         return "圖片修改完成！希望這次你喜歡！還可以繼續調整喔！加油！"
                     else:
                         user_image_generation_state[user_id] = 'can_modify'
                         return "圖片已生成但發送失敗。\n\n可能原因：圖片上傳服務(ImgBB/GCS)設定有誤。\n請檢查後台 Log。"
                 else:
        ...
        if detect_regenerate_image_intent(user_input):
            ...
            if success:
                ...
                if send_image_to_line(user_id, image_path, "你的圖片已更新！"):
                    user_image_generation_state[user_id] = 'can_modify'
                    return "圖片修改完成！希望這次你喜歡！還可以繼續調整喔！加油！Cheer up！"
                else:
                    user_image_generation_state[user_id] = 'can_modify'
        ...
        import traceback
        traceback.print_exc()
        return "哎呀！我遇到一點小問題...但別擔心！我們再試一次！加油！Cheer up！"
